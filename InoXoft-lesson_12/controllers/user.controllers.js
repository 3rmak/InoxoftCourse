...
const { passwordService, s3Service, usersQueryBuider: { findAllUsers } } = require('../services');
...
module.exports = {
  ...
  getAll: async (req, res, next) => {
    try {
      const { users, countOfPages } = await findAllUsers(req.query);

      res.json({ users, countOfPages });
    } catch (error) {
      next(error);
    }
  },
