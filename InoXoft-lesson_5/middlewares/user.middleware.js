...
module.exports = {
  ...
  isPostDataValid: (req, res, next) => {
    try {
      const { err, value } = userValidator.UserCreateValidator.validate(req.body);

      if (err) {
        throw new ErrorHandler(httpStatusCodes.Bad_Request, err.details[0].message);
      }
  ...
  isPatchDataValid: (req, res, next) => {
    try {
      const { err, value } = userValidator.UserPatchValidator.validate(req.body);
